...
class Paddle(Shape):
    def __init__(self,x,y,width=45,height=8,speed=17
    ,color="blue"):
    ...
    def limit(self,area):
        adjust = (max(self.x1,area.x1) - self.x1 or min(self.x2,area.x2)-self.x2)
        self.x1 += adjust
        self.x2 += adjust
    ...
    def draw(self,canvas):
        ...
        canvas.create_rectangle(self.x1,self.y1,self.x2,self.y2,fill=self.color,tag=self.name)
...
class Ball(Shape):
    ...
    def __init__(self,x,y,size=10,dx=5,dy=5,color="red"):
    ...
    def bound(self,target):
        ...
        if (self.dx > 0 and center_x <= target.x1 or self.dx < 0 and target.x2 <= center_x):
            self.dx *= -1
        if (self.dy > 0 and center_y <= target.y1 or self.dy < 0 and target.y2 <= center_y):
            self.dy *= -1
        return True
    
    def draw(self,canvas):
        canvas.delete(self.name)
        canvas.create_oval(self.x1,self.y1,self.x2,self.y2,fill=self.color,tag=self.name)
...
class Block(Shape):
    ...
    def __init__(self,x,y,width,height,gap_x=0,gap_y=0,center=False,color="orange",point=1):
        super().__init__(x+gap_x,y+gap_y,width - gap_x*2,height - gap_y*2,center=center)
        self.point = point
        self.color = color
...
class Blocks:
    ...
    def __init__(self,x,y,width,height,columns=12,rows=None):
        ...
        w=width // columns
        h=height // len(rows)
        self.blocks = [Block(x+dx,y+dy,w,h,gap_x=6,gap_y=12,color=rows.color,point=rows.point) for dy,rows in zip(range(0,h*len(rows)+1,h),rows) for dx in range(0,w*columns+1,w)]
...
class Score:
    ...
    def draw(self,canvas):
        canvas.delete(self.name)
        canvas.create_text(self.x,self.y,text=f"Score={self.score}",font=self.FONT,tag=self.name)
